...
class Linear(nn.Linear, LoraLayer):
    ...
    def merge(self):
        ...
        if self.r[self.active_adapter] > 0:
            ...
            # flag to True.
            self.weight.data += (self.lora_B[self.active_adapter] @ self.lora_A[self.active_adapter], True) * self.scaling[self.active_adapter] ### YOUR CODE HERE ###
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            result += self.lora_B[self.active_adapter](self.lora_dropout[self.active_adapter](self.lora_A[self.active_adapter](x))) ### YOUR CODE HERE ###
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
